-- ============================================================
-- DB PERFORMANCE BENCHMARK SUITE FOR WP-A3.2.0 (MIGRATION 021)
-- ============================================================
-- Evaluates query plan shape, buffer usage, and execution characteristics:
--   Pattern A: Direct per-row private.can_org(organization_id, ...)
--   Pattern B: Set-based IN predicate via private.authorized_orgs_for_capability(...)
--   Pattern C: Set-based JOIN via private.authorized_orgs_for_capability(...)
--
-- Uses 10,000 synthetic records in a temporary table across multiple tenants.
...
BEGIN
  ...
  INSERT INTO auth.users (
    ...
    'authenticated',
    'authenticated',
    'bench_user_wp_a320_' || v_synth_auth_id::text || '@mica.test',
  ...
  -- Note: is_local = true ensures setting automatically resets upon ROLLBACK
  ...
  -- Populate 10,000 synthetic records:
  ...
  RAISE NOTICE 'Benchmark fixture initialized: 10,000 synthetic records created across 3 organizations.';
END $$;


-- ============================================================
-- BENCHMARK QUERY 1: Pattern A (Direct Per-Row can_org Invocation)
-- ============================================================
-- Expected plan shape: Sequential scan with per-row function evaluation filter.
...
-- ============================================================
-- BENCHMARK QUERY 2: Pattern B (Set-Based IN Subquery via authorized_orgs_for_capability)
-- ============================================================
-- Expected plan shape: Index/Bitmap scan with single InitPlan execution.
EXPLAIN (ANALYZE, BUFFERS, VERBOSE)
SELECT COUNT(*), SUM(amount)
FROM temp_bench_financial_records
...
-- ============================================================
-- BENCHMARK QUERY 3: Pattern C (Set-Based Semi-Join via authorized_orgs_for_capability)
-- ============================================================
-- Expected plan shape: Nested loop / Hash join using index on organization_id.
EXPLAIN (ANALYZE, BUFFERS, VERBOSE)
