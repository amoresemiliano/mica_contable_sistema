...
-- ============================================================
-- MIGRATION 013.1: FIX FINANCIAL RPC CONTRACT
-- ============================================================
...
BEGIN
  ...
  -- Validar Metadatos de Archivo Server-Side
  ...
  LOOP
    ...
    IF NOT v_is_invalid THEN
      ...
      IF v_import_record.source_type = 'PAYROLL_ACONPY' THEN
        v_periodo := v_norm->>'periodo';
        IF v_periodo IS NULL OR v_periodo = '' THEN
          v_is_invalid := TRUE;
        ELSE
          v_computed_identity_key := jsonb_build_array('PAYROLL_ACONPY', v_periodo)::text;
          v_remunerativo := ROUND(COALESCE((v_norm->>'remunerativo')::numeric, 0), 2);
          v_noremunerativo := ROUND(COALESCE((v_norm->>'noRemunerativo')::numeric, 0), 2);
          v_anticipos := ROUND(COALESCE((v_norm->>'anticipos')::numeric, 0), 2);
          v_sac := ROUND(COALESCE((v_norm->>'sacProporcional')::numeric, 0), 2);
          v_sindicato := ROUND(COALESCE((v_norm->>'aporteSindicalObligatorio')::numeric, 0), 2);
          v_faecys := ROUND(COALESCE((v_norm->>'faecys')::numeric, 0), 2);
          v_neto := ROUND(COALESCE((v_norm->>'sueldoNeto')::numeric, 0), 2);

          v_computed_fingerprint := ENCODE(extensions.digest(
            v_remunerativo::text || '|' || v_noremunerativo::text || '|' || v_anticipos::text || '|' || v_sac::text || '|' || v_sindicato::text || '|' || v_faecys::text || '|' || v_neto::text,
            'sha256'
          ), 'hex');
        END IF;
      ...
      ELSIF v_import_record.source_type = 'BANK_STATEMENT_BBVA' THEN
        ...
        IF v_fecha_raw ~ '^\d{4}-\d{2}-\d{2}$' THEN v_fecha := v_fecha_raw::DATE;
        ELSIF v_fecha_raw ~ '^\d{2}/\d{2}/\d{4}$' THEN v_fecha := TO_DATE(v_fecha_raw, 'DD/MM/YYYY');
        ELSE v_is_invalid := TRUE; END IF;

        v_fecha_valor_raw := TRIM(COALESCE(v_norm->>'fechaValor', ''));
        IF v_fecha_valor_raw = '' THEN v_fecha_valor := v_fecha;
        ELSIF v_fecha_valor_raw ~ '^\d{4}-\d{2}-\d{2}$' THEN v_fecha_valor := v_fecha_valor_raw::DATE;
        ELSIF v_fecha_valor_raw ~ '^\d{2}/\d{2}/\d{4}$' THEN v_fecha_valor := TO_DATE(v_fecha_valor_raw, 'DD/MM/YYYY');
        ELSE v_is_invalid := TRUE; END IF;

        v_referencia := TRIM(COALESCE(v_norm->>'referencia', ''));
        v_saldo := (v_norm->>'saldo')::numeric;
        v_monto := (v_norm->>'monto')::numeric;
        v_tipo := v_norm->>'tipo';
        ...
        IF NOT v_is_invalid THEN
          v_computed_fingerprint := ENCODE(extensions.digest(
            v_descripcion || '|' || TO_CHAR(v_fecha_valor, 'YYYY-MM-DD') || '|' || (v_norm->>'signals')::text,
            'sha256'
          ), 'hex');
        END IF;
      END IF;
    ...
    IF v_row_status IN ('ACCEPTED', 'POSSIBLE_AMENDMENT') THEN
      INSERT INTO public.eco_financial_movements (
        ...
        v_fecha, v_fecha_valor, v_periodo, v_descripcion, v_referencia, v_account_id, v_tipo, ROUND(v_monto, 2), ROUND(v_saldo, 2), v_caller_id
      );
