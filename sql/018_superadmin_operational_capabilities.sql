...
-- ============================================================
-- MIGRATION 018: SUPERADMIN OPERATIONAL CAPABILITY INHERITANCE
-- ============================================================
-- Expands authorization contracts for SUPERADMIN across all operational
-- and administrative RPCs while strictly maintaining tenant isolation rules:
-- - Tenant operational RPCs require a valid active organization context (private.org_id() IS NOT NULL)
-- - Global catalog administration RPCs allow SUPERADMIN/ADMIN global access
-- ============================================================


-- 1. IMPORT & PERSISTENCE PIPELINE (TENANT SCOPED)
...
BEGIN
  ...
  IF v_caller_role NOT IN ('UPLOADER', 'ADMIN', 'SUPERADMIN') THEN
    RAISE EXCEPTION 'Unauthorized: Requires UPLOADER, ADMIN, or SUPERADMIN role';
  END IF;
  ...
  LOOP
    ...
    IF v_computed_status = 'INVALID' OR (v_row_record->'errors' IS NOT NULL AND jsonb_array_length(v_row_record->'errors') > 0) THEN
      ...
      INSERT INTO public.eco_import_issues (
        ...
        v_record_id,
        'PARSE_ERROR',
        'Fila inválida o con errores de parseo',
...
BEGIN
  ...
  IF v_caller_role NOT IN ('UPLOADER', 'ADMIN', 'SUPERADMIN') THEN
    RAISE EXCEPTION 'Unauthorized: Requires UPLOADER, ADMIN, or SUPERADMIN role (got %)', v_caller_role;
  END IF;
...
REVOKE ALL ON FUNCTION public.persist_perceptions_batch(uuid, jsonb, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.persist_perceptions_batch(uuid, jsonb, jsonb) TO authenticated;
...
-- 2. REVIEW & CLASSIFICATION (TENANT SCOPED)
...
BEGIN
  ...
  IF v_caller_role NOT IN ('REVIEWER', 'ADMIN', 'SUPERADMIN') THEN
    RAISE EXCEPTION 'Unauthorized: Requires REVIEWER, ADMIN, or SUPERADMIN role';
  END IF;
...
-- 3. GLOBAL & TENANT CATALOG ADMINISTRATION
...
BEGIN
  ...
  IF v_org_id IS NOT NULL THEN
    INSERT INTO public.eco_audit_events (organization_id, event_type)
    VALUES (v_org_id, 'ARCA_CATALOG_UPSERTED');
  END IF;
END;
$$;
