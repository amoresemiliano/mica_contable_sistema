...
-- Note: pgcrypto extension is already pre-installed in schema 'extensions'

-- ============================================================
-- 1. MAQUINA DE ESTADOS Y EXTENSION DE ECO_SOURCE_IMPORTS
-- ============================================================

ALTER TABLE public.eco_source_imports
  ADD COLUMN IF NOT EXISTS source_type TEXT NOT NULL DEFAULT 'ARCA_RECIBIDOS',
  ADD COLUMN IF NOT EXISTS operation_type TEXT DEFAULT 'COMPRA',
  ADD COLUMN IF NOT EXISTS total_rows INTEGER DEFAULT 0,
  ADD COLUMN IF NOT EXISTS accepted_rows INTEGER DEFAULT 0,
  ...
  ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES public.eco_user_profiles(id);
...
ALTER TABLE public.eco_source_imports
  ADD CONSTRAINT chk_import_op_type
  CHECK (operation_type IN ('COMPRA', 'VENTA', 'PERCEPCION', 'BANCO', 'SUELDO'));

-- ============================================================
-- 2. EXTENSION DE ECO_SOURCE_FILES E IDEMPOTENCIA POR HASH (V4.1 NOT NULL)
-- ============================================================

ALTER TABLE public.eco_source_files
  ADD COLUMN IF NOT EXISTS organization_id UUID NOT NULL REFERENCES public.eco_organizations(id),
  ADD COLUMN IF NOT EXISTS original_name TEXT NOT NULL,
  ADD COLUMN IF NOT EXISTS storage_path TEXT NOT NULL,
  ADD COLUMN IF NOT EXISTS mime_type TEXT,
  ADD COLUMN IF NOT EXISTS size_bytes BIGINT NOT NULL,
  ADD COLUMN IF NOT EXISTS sha256_hash TEXT NOT NULL,
  ADD COLUMN IF NOT EXISTS source_type TEXT NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_source_files_org_hash
  ON public.eco_source_files (organization_id, sha256_hash);

-- ============================================================
-- 3. EXTENSION DE ECO_IMPORT_ROWS (V4.1 NOT NULL)
-- ============================================================

ALTER TABLE public.eco_import_rows
  ADD COLUMN IF NOT EXISTS organization_id UUID NOT NULL REFERENCES public.eco_organizations(id),
  ADD COLUMN IF NOT EXISTS source_row_number INTEGER NOT NULL,
  ADD COLUMN IF NOT EXISTS raw_payload JSONB,
  ADD COLUMN IF NOT EXISTS parse_status TEXT NOT NULL,
  ADD COLUMN IF NOT EXISTS errors JSONB DEFAULT '[]'::jsonb,
  ADD COLUMN IF NOT EXISTS warnings JSONB DEFAULT '[]'::jsonb;

ALTER TABLE public.eco_import_rows
  DROP CONSTRAINT IF EXISTS chk_row_parse_status;

ALTER TABLE public.eco_import_rows
  ADD CONSTRAINT chk_row_parse_status
  CHECK (parse_status IN ('ACCEPTED', 'INVALID', 'EXACT_DUPLICATE', 'POSSIBLE_AMENDMENT'));

-- ============================================================
-- 4. EXTENSION DE ECO_NORMALIZED_RECORDS Y UNICIDAD DE VERSION (V4.1 NOT NULL)
-- ============================================================

ALTER TABLE public.eco_normalized_records
  ADD COLUMN IF NOT EXISTS organization_id UUID NOT NULL REFERENCES public.eco_organizations(id),
  ADD COLUMN IF NOT EXISTS record_type TEXT NOT NULL DEFAULT 'ARCA_RECIBIDO',
  ADD COLUMN IF NOT EXISTS identity_key TEXT NOT NULL,
  ADD COLUMN IF NOT EXISTS fiscal_fingerprint TEXT NOT NULL,
  ADD COLUMN IF NOT EXISTS normalized_payload JSONB NOT NULL,
  ADD COLUMN IF NOT EXISTS fecha DATE,
  ADD COLUMN IF NOT EXISTS cuit TEXT,
  ADD COLUMN IF NOT EXISTS razon_social TEXT,
  ADD COLUMN IF NOT EXISTS comprobante TEXT,
  ADD COLUMN IF NOT EXISTS total NUMERIC(15,2),
  ADD COLUMN IF NOT EXISTS tipo_operacion TEXT NOT NULL,
  ADD COLUMN IF NOT EXISTS categoria TEXT,
  ADD COLUMN IF NOT EXISTS confirmada BOOLEAN DEFAULT FALSE;

CREATE UNIQUE INDEX IF NOT EXISTS idx_normalized_org_identity_fingerprint
  ON public.eco_normalized_records (organization_id, identity_key, fiscal_fingerprint);

CREATE INDEX IF NOT EXISTS idx_normalized_org_lookup
  ON public.eco_normalized_records (organization_id, fecha, cuit);

-- RLS explícito en eco_normalized_records
DROP POLICY IF EXISTS "Records viewable by org" ON public.eco_normalized_records;

CREATE POLICY "Records viewable by org"
ON public.eco_normalized_records
FOR SELECT
TO authenticated
USING (organization_id = private.org_id());

-- ============================================================
-- 5. EXTENSION DE ECO_IMPORT_ISSUES Y MODELO DE ACCESO (V4.1 NOT NULL)
-- ============================================================

ALTER TABLE public.eco_import_issues
  ALTER COLUMN record_id DROP NOT NULL,
  ADD COLUMN IF NOT EXISTS organization_id UUID NOT NULL REFERENCES public.eco_organizations(id),
  ADD COLUMN IF NOT EXISTS import_id UUID NOT NULL REFERENCES public.eco_source_imports(id),
  ADD COLUMN IF NOT EXISTS row_id UUID REFERENCES public.eco_import_rows(id),
  ADD COLUMN IF NOT EXISTS issue_type TEXT NOT NULL DEFAULT 'PARSE_ERROR',
  ADD COLUMN IF NOT EXISTS message TEXT NOT NULL,
  ADD COLUMN IF NOT EXISTS details JSONB;

-- Actualización de Helper private.has_issue_access
...
AS $$
  SELECT EXISTS (
    ...
    WHERE iss.id = p_issue_id AND iss.organization_id = private.org_id()
  );
$$;

-- RLS explícito en eco_import_issues
DROP POLICY IF EXISTS "Issues viewable by org" ON public.eco_import_issues;

CREATE POLICY "Issues viewable by org"
ON public.eco_import_issues
FOR SELECT
TO authenticated
USING (organization_id = private.org_id());

-- Actualización de RPC public.resolve_issue
...
-- ============================================================
-- 6. RPC PRE-CHECK DE ARCHIVO DUPLICADO (PRE-UPLOAD)
-- ============================================================
...
BEGIN
  ...
  WHERE organization_id = v_org_id AND sha256_hash = p_sha256_hash;
...
-- ============================================================
-- 8. RPC TRANSACCIONAL PERSIST_IMPORT_BATCH
-- ============================================================
...
-- ============================================================
-- 9. LECTURA SEGURA DETERMINISTICA
-- ============================================================
